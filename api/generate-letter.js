  ...
  try {
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        messages: [{ role: "user", content: `Write a warm anonymous positive letter to a stranger. Theme: "${label}" — ${prompt}. Rules: 3-5 sentences, heartfelt, genuine, no names or handles, end with a fitting emoji. Write the letter only, nothing else.` }],
      }),
    });
    d = await r.json();
  } catch (e) {
